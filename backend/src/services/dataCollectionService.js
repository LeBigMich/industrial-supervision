...
class DataCollectionService {
    ...
    async collectData() {
        try {
            const [parameters] = await db.query('SELECT * FROM parameters WHERE is_active = TRUE');
            
            for (const param of parameters) {
                try {
                    const value = await modbusService.readRegister(param.plc_ip, param.modbus_address);
                    ...
                    if (value !== null && value !== undefined) {
                        ...
                        if (param.min_value !== null && value < param.min_value) {
                            await this.triggerAlert(param.id, 'MIN', `Valeur ${value} en-dessous du minimum ${param.min_value}`);
                        }
                        if (param.max_value !== null && value > param.max_value) {
                            await this.triggerAlert(param.id, 'MAX', `Valeur ${value} au-dessus du maximum ${param.max_value}`);
                        }
                    }
                } catch (error) {
